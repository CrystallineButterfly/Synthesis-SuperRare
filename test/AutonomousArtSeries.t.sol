...
contract AutonomousArtSeriesTest is Test {
    AutonomousArtSeries private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_superrare");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new AutonomousArtSeries(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        AutonomousArtSeries freshController = new AutonomousArtSeries(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
